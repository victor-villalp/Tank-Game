...
public class GameWorld extends JPanel {
    ...
    private void init(){
        ...
        try {
            ...
            powerUpImg = ImageIO.read(getClass().getResource("/resources/Wall2.gif"));
    ...
    private void gameUpdate() {
        for(int i=0; i < gameObjects.size(); i++){
            if(gameObjects.get(i) instanceof Bullet){
                if(((Bullet) gameObjects.get(i)).isAlive()) {
            ...
            if(gameObjects.get(i) instanceof Tank){
                if(((Tank)gameObjects.get(i)).isAlive()){
                    gameObjects.get(i).update();
                } else {
                    gameObjects.remove(i);
                    i--;
                ...
                for(int i = 0; i < tokens.length; i++){
                    switch(tokens[i]) {
                        ...
                        case "2":
                            ...
                            break;
                        case "3":
                            t1 = new Tank(i*32,j*32,0,0,0, tankImg, bulletImg, "Tank1", this);
                            ...
                            break;
                        case "4":
                            t2 = new Tank(i*32,j*32,0,0,0, tankImg, bulletImg, "Tank2", this);
    ...
    private void checkCollisions(){
        /*for(int i=1; i < gameObjects.size(); i++){
            for(int j=0; j < gameObjects.size(); j++){
                ...
                if(i != j) {
                    if (((GameObject) co1).getRec().getBounds().intersects(((GameObject) co1).getRec().getBounds())) {
                        co1.collision(co2);
                        co2.collision(co1);
                    }
                }
    ...
    public void paintComponent(Graphics g) {
        ...
        // Draw all objects
        gameObjects.forEach(object -> object.drawImage(buffer));
        ...
        g2.drawString("Tank1 lives: " , 10, 28);
        g2.drawString("Tank2 lives: " , SCREEN_WIDTH / 2 + 10, 28);
